...
from backend.services.feature_filter import get_initial_features, filter_analytics, HINTS_BY_DIFFICULTY, HARD_FEATURES, MEDIUM_FEATURES, EASY_FEATURES, get_hint_feature
...
@router.post("/")
async def start_session(difficulty: DifficultyLevel, db: AsyncSession = Depends(get_db)):
    ...
    hints_config = HINTS_BY_DIFFICULTY[difficulty]
    session = GameSession(
    ...
    visible = get_initial_features(difficulty)
    return {
...
async def submit_guess(session_id: uuid.UUID, answer: str, db: AsyncSession = Depends(get_db)):
    session = await db.get(GameSession, session_id)
    if not session:
        raise HTTPException(status_code=404)
    if session.correct is not None:
    ...
    correct = answer.lower() == audio_source.audio_source_name.lower()
    ...
    return {
        "correct": correct,
        "correct_answer": audio_file.category,
        "score": session.score
    }
...
async def reveal(session_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    return {
        ...
        "subcategory": audio_file.subcategory,
